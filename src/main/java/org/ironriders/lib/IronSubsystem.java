...
public abstract class IronSubsystem extends SubsystemBase {
    ...
    private final String diagnosticName = this.getClass().getSimpleName().replaceAll("Subsystem$", ""); // the dollar
                                                                                                        // sign
    // here is regex
    // to look for
    // Subsystem, not
     ...
     * SmartDashboard with
     * the prefix `debug/Subsystems/{subsystem name}/`. If the value is a Command,
     * it will also be
     ...
     * DriverStation} with the time and subsystem name; Sends a notification to
     * {@linkplain
     * org.ironriders.lib.Elastic#sendNotification(Notification) Elastic} with level
     * {@link
